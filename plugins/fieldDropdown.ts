...
/**
 * 改自 blockly\core\field_dropdown
 */
export class dropdownWithInput extends Blockly.Field<string> {
    /**
    ...
    protected menuGenerator_?: Blockly.MenuGenerator;
    ...
    private selectedOption!: Blockly.MenuOption;
    ...
    protected static IMAGE_Y_PADDING = dropdownWithInput.IMAGE_Y_OFFSET * 2;
     ...
     *     function which generates these options. Also accepts Blockly.Field.SKIP_SETUP
    ...
    constructor(menuGenerator: typeof Blockly.Field.SKIP_SETUP);
    constructor(
        ...
        if (menuGenerator === Blockly.Field.SKIP_SETUP) return;
    ...
    override loadState(state: unknown) {
        if (this.loadLegacyState(dropdownWithInput, state)) {
            return;
        }
        if (this.isOptionListDynamic()) {
            this.getOptions(false);
        }
        this.setValue(state as string);
    }
    ...
    override initView() {
        ...
        if (this.fieldGroup_) {
            ...
            Blockly.utils.dom.addClass(this.fieldGroup_, 'blocklyDropdownField');
        }
    ...
    override isFullBlockField(): boolean {
        return !this.shouldAddBorderRect_() && !!this.getSourceBlock()?.isSimpleReporter();
    }

    /**
     * Whether or not the dropdown should add a border rect.
     *
     * On a shadow block the dropdown sits flush on the rounded shadow body
     * (no separate chip), matching Scratch. On a regular block a rounded
     * border rect is drawn as the dropdown chip.
     *
     * @returns True if the dropdown field should add a border rect.
     */
    protected shouldAddBorderRect_(): boolean {
        return !this.getSourceBlock()?.isShadow();
    }
    ...
    private dropdownCreate() {
        ...
        Blockly.utils.aria.setState(
            this.getFocusableElement(),
            Blockly.utils.aria.State.CONTROLS,
            this.menu_.getId(),
        );
        ...
        for (let i = 0; i < options.length; i++) {
            ...
            if (option === dropdownWithInput.SEPARATOR) {
                menu.addChild(new MenuSeparator() as unknown as Blockly.MenuItem);
                continue;
            }
            ...
            const menuItem = new Blockly.MenuItem(content, value, ariaLabel);
     ...
     *
     * @param menu The Blockly.Menu component clicked.
     * @param menuItem The MenuItem selected within menu.
     */
    protected onItemSelected_(_menu: Blockly.Menu, menuItem: Blockly.MenuItem) {
    ...
    getOptions(useCache?: boolean): Blockly.MenuOption[] {
        ...
        this.generatedOptions = (this.menuGenerator_ as () => Blockly.MenuOption[])();
     ...
     */
    setOptions(menuGenerator: Blockly.MenuGenerator) {
     ...
     */
    private renderSelectedImage(imageJson: Blockly.ImageProperties) {
        const block = this.getSourceBlock();
        if (!block) {
            throw new Blockly.UnattachedFieldError();
        }
        this.imageElement!.style.display = '';
        this.imageElement!.setAttributeNS(Blockly.utils.dom.XLINK_NS, 'xlink:href', imageJson.src);
        ...
        const height = Math.max(
            ...
            imageHeight + dropdownWithInput.IMAGE_Y_PADDING,
        );
        ...
        if (this.svgArrow) {
            ...
            arrowWidth = Blockly.utils.dom.getTextWidth(this.arrow as SVGTSpanElement);
        }
        this.size_ = new Blockly.utils.Size(imageWidth + arrowWidth + xPadding * 2, height);
    ...
    private renderSelectedText() {
        ...
        const textWidth = Blockly.utils.dom.getTextWidth(this.getTextElement());
        const xPadding = hasBorder ? this.getConstants()!.FIELD_BORDER_RECT_X_PADDING : 0;
        let arrowWidth = 0;
        if (this.svgArrow) {
            arrowWidth = this.positionSVGArrow(
        ...
        this.size_ = new Blockly.utils.Size(textWidth + arrowWidth + xPadding * 2, height);
    ...
    protected override getText_(): string | null {
        ...
        if (isImageProperties(option)) {
            return option.alt;
        } else if (typeof HTMLElement !== 'undefined' && option instanceof HTMLElement) {
    ...
    /**
     * Construct a dropdownWithInput from a JSON arg object.
     *
     ...
     * @internal
     */
    static override fromJson(options: Blockly.FieldDropdownFromJsonConfig): dropdownWithInput {
        if (!options.options) {
            throw new Error(
        ...
        // `this` might be a subclass of dropdownWithInput if that class doesn't
    ...
    protected trimOptions(options: Blockly.MenuOption[]): {
        ...
        const trimmedOptions = options.map((option): Blockly.MenuOption => {
            if (option === dropdownWithInput.SEPARATOR) {
            ...
            if (typeof label === 'string') {
                const trimmedLabelOption: Blockly.MenuOption = [
                    Blockly.utils.parsing.replaceMessageReferences(label),
                    value,
                ];
                if (ariaLabel) {
                    trimmedLabelOption.push(
                        Blockly.utils.parsing.replaceMessageReferences(ariaLabel),
                    );
                }
            ...
            const imageLabel = isImageProperties(label)
                ? { ...label, alt: Blockly.utils.parsing.replaceMessageReferences(label.alt) }
                : label;
            const imageOptions: Blockly.MenuOption = [imageLabel, value];
            if (ariaLabel) {
                imageOptions.push(Blockly.utils.parsing.replaceMessageReferences(ariaLabel));
            }
            return imageOptions;
        });
        ...
        const shortest = Blockly.utils.string.shortestStringLength(stringLabels);
        const prefixLength = Blockly.utils.string.commonWordPrefix(stringLabels, shortest);
        const suffixLength = Blockly.utils.string.commonWordSuffix(stringLabels, shortest);
    ...
    private applyTrim(
        ...
        return options.map(([text, value, ariaLabel]) => {
            const trimmedText = text.substring(prefixLength, text.length - suffixLength);

            return ariaLabel !== undefined ? [trimmedText, value, ariaLabel] : [trimmedText, value];
        });
    }
     ...
     */
    protected validateOptions(options: Blockly.MenuOption[]) {
        if (!Array.isArray(options)) {
            throw TypeError('dropdownWithInput options must be an array.');
        }
        if (!options.length) {
            throw TypeError('dropdownWithInput options must not be an empty array.');
        }
        let foundError = false;
        for (let i = 0; i < options.length; i++) {
            const option = options[i];
            if (!Array.isArray(option) && option !== dropdownWithInput.SEPARATOR) {
                foundError = true;
                console.error(
                    `Invalid option[${i}]: Each dropdownWithInput option must be an array or
          the string literal 'separator'. Found: ${option}`,
                );
            } else if (typeof option[1] !== 'string') {
                foundError = true;
                console.error(
                    `Invalid option[${i}]: Each dropdownWithInput option id must be a string.
          Found ${option[1]} in: ${option}`,
                ...
                foundError = true;
                console.error(
                    `Invalid option[${i}]: Each dropdownWithInput option must have a string
          label, image description, or HTML element. Found ${option[0]} in: ${option}`,
                );
            } else if (option[2] && typeof option[2] !== 'string') {
                foundError = true;
                console.error(
                    `Invalid option[${i}]: Each dropdownWithInput option ARIA label must be a string.
          Found ${option[2]} in: ${option}`,
                );
            }
        }
        if (foundError) {
            throw TypeError('Found invalid dropdownWithInput options.');
        }
    }
    ...
    override getAriaTypeName(): string | null {
        return this.ariaTypeName || Blockly.Msg['ARIA_TYPE_FIELD_DROPDOWN'];
    }
    ...
    override getAriaValue(): string {
        ...
        // defensive safeguard.
        return this.getSelectedAriaLabel() || this.getText() || Blockly.Msg['FIELD_LABEL_EMPTY'];
    }
    ...
    override recomputeAriaContext(): boolean {
        ...
        Blockly.utils.aria.setState(focusableElement, Blockly.utils.aria.State.HASPOPUP, 'listbox');
        Blockly.utils.aria.setState(
            focusableElement,
            Blockly.utils.aria.State.EXPANDED,
            !!this.menu_,
     ...
     */
    private computeOptionAriaLabel(option: Blockly.MenuOption, index: number): string {
        if (option === dropdownWithInput.SEPARATOR) return '';
        ...
        } else if (typeof HTMLElement !== 'undefined' && label instanceof HTMLElement) {
        ...
        return Blockly.Msg['FIELD_LABEL_OPTION_INDEX'].replace('%1', String(index + 1));
    }
}

function isImageProperties(obj: unknown): obj is Blockly.ImageProperties {
...
class MenuSeparator {
    ...
    createDom(): HTMLHRElement {
        ...
        Blockly.utils.aria.setRole(this.element, Blockly.utils.aria.Role.SEPARATOR);
