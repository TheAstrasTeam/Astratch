...
export class Grid {
    ...
    private populateItems(options: MenuOption[]) {
        ...
        for (const [index, item] of options.entries()) {
            ...
            const gridItem = new GridItem(row, content, value, (selectedItem: GridItem) => {
    ...
    private onKeyDown(e: KeyboardEvent) {
        if (!this.items.length || e.shiftKey || e.ctrlKey || e.metaKey || e.altKey) {
            return;
        }
    ...
    private moveFocus(index: number, relative: boolean) {
        ...
        targetItem.focus();
        utils.aria.setState(this.root, utils.aria.State.ACTIVEDESCENDANT, targetItem.getId());
    }
