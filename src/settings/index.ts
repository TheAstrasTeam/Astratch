    ...
    Settings.registerMany([
        {
            ...
            category: 'general',
            label: 'gui:settings.userName',
            type: 'text',
            description: 'gui:settings.userName.description',
        },
        {
            key: 'language',
            defaultValue: i18next.language,
            category: 'general',
            label: 'gui:settings.language',
            type: 'select',
        ...
        },
        {
            ...
            category: 'appearance',
            label: 'gui:settings.guiThemeMode',
            type: 'select',
            options: Object.keys(guiThemes).map(k => ({ value: k, label: `gui:theme.${k}` })),
        },
        {
            key: 'guiThemeAccent',
            defaultValue: DEFAULT_GUITHEME_MAP.accent,
            category: 'appearance',
            label: 'gui:settings.guiThemeAccent',
            type: 'select',
            options: Object.keys(guiAccents).map(k => ({ value: k, label: `gui:accent.${k}` })),
        },
    ]);
