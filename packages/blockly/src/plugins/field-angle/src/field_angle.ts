...
export class FieldAngle extends Blockly.FieldNumber {
     ...
     */
    static readonly RADIUS: number = FieldAngle.HALF * 0.75 - 1;
    ...
    override initView() {
        ...
        if (this.symbol) {
            ...
            this.symbolElement = Blockly.utils.dom.createSvgElement(Blockly.utils.Svg.TSPAN, {});
    ...
    protected override showEditor_(e?: Event) {
        ...
        // getSourceBlock 获取的是输入框的颜色，
        // 需要用 getParent 获取实际积木的颜色。
        // 通常来说只要符合规范是没问题的，
        // 但对于某些情况，还是需要获取 SourceBlock 而不是
        // 更上层的积木。
        const sourceBlock = this.getSourceBlock()?.getParent() ?? this.getSourceBlock();
    ...
    private dropdownCreate(): SVGSVGElement {
        ...
        const minValueDegrees = Blockly.utils.math.toDegrees(this.fieldAngleToRadians(this.min_));
        const maxValueDegrees = Blockly.utils.math.toDegrees(this.fieldAngleToRadians(this.max_));
        ...
        const drawTicks = (tickAngle: number, length: number) => {
            ...
            for (let angle = min; angle <= max; angle += tickAngle) {
                Blockly.utils.dom.createSvgElement(
                    Blockly.utils.Svg.LINE,
                    {
                        x1: FieldAngle.HALF + FieldAngle.RADIUS * 0.9,
                        y1: FieldAngle.HALF,
                        x2: FieldAngle.HALF + FieldAngle.RADIUS * 0.9 - length,
                        y2: FieldAngle.HALF,
        ...
        drawTicks(minorTickAngle, 7);

        // The angle picker is different from other fields in that it updates on
        // mousemove even if it's not in the middle of a drag.  In future we may
        // change this behaviour.
        this.boundEvents.push(Blockly.browserEvents.conditionalBind(svg, 'click', this, this.hide));
 ...
 *   - symbol: '°'
 */
// @ts-expect-error 插件不符合项目要求，但需要启用
export enum Mode {
    COMPASS = 'compass',
